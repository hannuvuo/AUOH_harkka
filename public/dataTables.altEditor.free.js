(function(factory) {
    ...
    $.extend(altEditor.prototype, {
      ...
      _setup: function() {
        ...
        this.language =
          DataTable.settings.values().next().value.oLanguage.altEditor || {};
        ...
        var modal =
          ...
          '<button style="margin: initial;" type="button" class="close" data-dismiss="modal" aria-label="' +
          ...
          if (dt.row({ selected: true }).index() != null)
            selectedCellData = dt
              .cell(
                ...
                dt.column("th:contains('" + target.attr("name") + "')").index()
              )
              .data();
      ...
      _openEditModal: function() {
        ...
        for (var j in columnDefs) {
          ...
          $(selector)
            .find(jquerySelector)
            .val(this._quoteattr(selectedValue));
          $(selector)
            .find(jquerySelector)
      ...
      _openDeleteModal: function() {
        ...
        for (var j in columnDefs) {
          if (columnDefs[j].type.indexOf("hidden") >= 0) {
            ...
            data +=
              "<div style='margin-left: initial;margin-right: initial;' class='form-group row'><label for='" +
        ...
        $(selector).on("show.bs.modal", function() {
          var btns =
            '<button type="button" data-content="remove" class="btn btn-default" data-dismiss="modal">' +
            that.language.modalClose +
            "</button>" +
            '<button type="submit"  data-content="remove" class="btn btn-danger" id="deleteRowBtn">' +
      ...
      _deleteRow: function() {
        ...
        for (var i = 0; i < dt.context[0].aoColumns.length; i++) {
          ...
          var name = dt.context[0].aoColumns[i].data
      ...
      createDialog: function(
        ...
        for (var j in columnDefs) {
          ...
          if (columnDefs[j].type.indexOf("hidden") >= 0) {
            data +=
              "<input type='hidden' id='" + columnDefs[j].name + "' ></input>";
          } else {
            ...
            //Adding Text Area
              ...
              data +=
                ...
                this._quoteattr(columnDefs[j].title) +
                "'rows='" +
                this._quoteattr(columnDefs[j].rows) +
                "' cols='" +
                this._quoteattr(columnDefs[j].cols) +
                "'>" +
                "</textarea>";
            }
            // Adding text-inputs and errorlabels, but also new HTML5 typees (email, color, ...)
              ...
              data +=
                ...
                (columnDefs[j].maxLength == false
                  ? ""
                  : " maxlength='" + columnDefs[j].maxLength + "'") +
                " style='overflow:hidden'  class='form-control  form-control-sm' value=''>";
            }
            data +=
              "<label id='" +
        ...
        $(selector).on("show.bs.modal", function() {
          var btns =
            ...
            formName +
            '" data-content="remove" class="btn btn-primary" id="' +
            buttonClass +
            '">' +
            buttonCaption +
            "</button>";
